...
export function isTableEmpty() {
  cy.get('.phl__table .q-table__bottom--nodata').should(
    'contain',
    'No data available'
  );
}
...
function withinPortal({
  ...
  console.log(subject)
  return cy
    .get(selector, { withinSubject: subject })
...
export function checkRowHp(slug: string, hp: number) {
  cy.get(`.phl__table .phl__actions[data-slug="${slug}"]`)
    .parent()
    .get('td')
    .eq(2)
    .should('contain', '' + hp);
