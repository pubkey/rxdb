...
<!--
# RxDB Frontend JavaScript Database

## Why you might want to store data in the frontend
- Data is Offline accessible
- Use the database for Caching
- Decrease initial application start time because the data is already there
- Store local data password encrypted
- Using a local database for state management instead of redux/ngrx
- Low latency queries that run fully local
- Building realtime applications is easier with local data
- Frontend databases are easier to integrate with JavaScript frameworks like angular, react.js vuejs and svelte
- Replicating database state can be easier to implement compared to complex REST routes
- Frontend databases can scale better because the queries run on the users device instead of the server

## Why SQL databases are not a good fit for the frontend
- Push/Pull based vs reactive
- Initialization time and performance
- Build Size of Server Side databases is too big for browser applications
- Frontend Databases directly uses the browser APIs like IndexedDB, OPFS, Webworker

## Why RxDB is a good fit for the frontend

- Made in JavaScript, optimized for JavaScript applications
- NoSQL (json) Documents  are a better fit for UIs because JavaScript works on JSON objects
...
- Storing Documents Compressed
- Built in Multi-tab support
- Replication Algorithm (open source) can be made compatible with any backend (own servers, firebase, CouchDB, NATS, WebSocket)
- Flexible storage layer, reuse the same code across Electron.js, react-native, hybrid apps (capacitor.js), browsers
- Handling of schema changes even when the data is stored on many client devices.


## Follow Up
- RxDB Quickstart
- RxDB Github Repostiory
-->
