...
Now we have a single plugin `dev-mode` that contains all these checks and development helper functions. I also moved many other checks out of the core-module into [dev-mode](../dev-mode.md).
...
In dev-mode, the GraphQL-[replication](../replication.md) will run a [schema validation](../schema-validation.md) of each document that comes from the server before it is saved to the database.
...
* The [GraphQL replication](../replication-graphql.md) is now out of beta mode
