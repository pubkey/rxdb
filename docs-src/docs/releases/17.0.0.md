...
import { PERFORMANCE_DATA_BROWSER, PERFORMANCE_DATA_NODE, PERFORMANCE_METRICS } from '@site/src/components/performance-data';
...
<PerformanceChart title="IndexedDB, OPFS & SQLite" data={[...PERFORMANCE_DATA_BROWSER.filter(d => ['IndexedDB', 'OPFS (Worker)'].includes(d.name)), ...PERFORMANCE_DATA_NODE.filter(d => d.name === 'SQLite (node:sqlite)')]} metrics={PERFORMANCE_METRICS} />
