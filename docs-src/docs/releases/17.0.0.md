...
RxDB 17 includes several changes that reduce CPU usage, memory footprint, and storage size across read and write operations.

### Faster bulk inserts

The bulk insert hot path has been optimized by inlining per-document helper calls, replacing `hasOwnProperty` checks with direct property access, skipping unnecessary cloning on inserts, and using counter-based event IDs instead of string-generated ones.

### Optimized `$in` queries on the primary key

Calling `find()` with `$in` on the primary key now correctly routes to the fast `findDocumentsById` path instead of falling back to a full storage query. [#7993](https://github.com/pubkey/rxdb/pull/7993)

### Binary storage format for IndexedDB

IndexedDB RxStorage now stores data in a binary format, reducing disc space usage compared to the previous JSON-based storage.

### Replaced `appendToArray()` with `Array.concat()`

Internal array operations switched from `appendToArray()` to `Array.concat()`, which modern JavaScript engines optimize at the native level.

### Smaller replication footprint

Pull-only replications no longer store server metadata on the client, reducing both memory consumption and storage writes for read-only sync scenarios.

### Write batching with `waitBeforePersist`

The new `waitBeforePersist` option on `ReplicationPushOptions` delays upstream persistence cycles so that writes from multiple collections can be batched together. This also allows deferring writes until the CPU is idle. [#7872](https://github.com/pubkey/rxdb/issues/7872)

### Memory leak fixes

- OPFS RxStorage memory and cleanup leaks have been fixed.
- The memory-mapped storage now correctly purges deleted documents.
- A memory leak in `migratePromise()` has been resolved. [#7787](https://github.com/pubkey/rxdb/pull/7787)
- Old collection meta documents are now deleted during migration. [#7792](https://github.com/pubkey/rxdb/pull/7792)
