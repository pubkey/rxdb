...
# <IconQuickstart style={{ width: '45px', height: '45px', verticalAlign: 'text-bottom', marginRight: '10px' }} /> RxDB 17.0.0 - Local-First to the Moon

RxDB 17 focuses on features for Vibe-Coders, **better reactivity**, **improved debugging and DX-for-LLMs**, performance and **important breaking storage fixes**, while also graduating several long-standing plugins out of beta.
...
<div style={{textAlign: 'justify'}}>
  <a href="../replication-google-drive.html">
    <img src="../files/icons/google-drive.svg" alt="Google Drive Sync" width="60" className="img-in-text-right" style={{ backgroundColor: 'transparent' }} />
  </a>


These plugins are the future for vibe-coded applications. They allow developers to create fully functional, cross-device applications without hosting any backend servers or managing database infrastructure. Instead of storing user data on a central server, the application syncs directly to the user's own Google or Microsoft account.
...
## <IconVibeCoding style={{ width: '35px', height: '35px', verticalAlign: 'text-bottom', marginRight: '10px' }} /> Improvements for our Vibe-Coders
...
## <IconDevicePhone style={{ width: '35px', height: '35px', verticalAlign: 'text-bottom', marginRight: '10px' }} /> Expo Filesystem RxStorage
...
- **ADD** enforce maximum length for indexes and primary keys (`maxLength: 2048`) because really big maxLength values impact performance.


## <IconExperiment style={{ width: '35px', height: '35px', verticalAlign: 'text-bottom', marginRight: '10px' }} /> Plugins Graduating from Beta
...
- The GitHub repository **no longer contains prebuilt `dist` files**. Install RxDB from npm or run the build scripts locally if you need them.
