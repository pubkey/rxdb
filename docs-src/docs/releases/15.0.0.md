...
The release [15.0.0](https://rxdb.info/releases/15.0.0.html) is used for major refactorings in the migration plugins and performance improvements of the [RxStorage](../rx-storage.md) implementations.
...
Also it now handles multi-instance runtimes correctly. If multiple browser tabs are open, only one of them (per [RxCollection](../rx-collection.md)) will run the migration.
...
This was changed in [v14](./14.0.0.md) for a normal [RxDocument](../rx-document.md).$ which emits RxDocument instances. Same is now also done for [local documents](../rx-local-document.md).
...
We now have a [dev-mode](../dev-mode.md) check to throw a helpful error message if $refs are used in the schema.
