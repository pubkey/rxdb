...
Most of the [GraphQL replication](../replication-graphql.md) code has been replaced by using the [replication](../replication.md) primitives plugin internally.
...
By default, the query will be sent to [RxStorage](../rx-storage.md), where a query planner will determine which one of the available indexes must be used.
...
const queryResults = await myCollection
    .find({
       ...
       * performance. This could not be known by
       * the query planner which might have
...
- [dev-mode](../dev-mode.md): Add check to ensure all top-level fields in a query are defined in the schema.
- Support for array field based indexes like `data.[].subfield` was removed, as it anyway never really worked.
- Refactored the usage of [RxCollection](../rx-collection.md).storageInstance to ensure all hooks run properly.
- Refactored the [encryption](../encryption.md) plugin so no more plugin specific code is in the RxDB core.
...
- [Attachments](../rx-attachment.md) are now internally handled as string instead of `Blob` or `Buffer`
