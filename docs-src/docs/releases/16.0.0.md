...
Data stored in the previous version `15` is compatible with the code of the new version `16` for most [RxStorage](../rx-storage.md) implementation. So migration will be easy. 
...
- FIX ([GraphQL replication](../replication-graphql.md)) datapath must be equivalent for pull and push [#6019](https://github.com/pubkey/rxdb/pull/6019)
...
The `ignoreDuplicate` flag is only useful for tests and should never be used in production. We now throw an error if it is set to `true` in non-[dev-mode](../dev-mode.md).
...
- Having a lot of documents pulled in the [replication](../replication.md) could in some cases slow down the database initialization because `upstreamInitialSync()` did not set a checkpoint and each time checked all documents if they are equal to the master.
...
- Fix IndexedDB bug: Some people had problems with the [IndexedDB RxStorage](../rx-storage-indexeddb.md) that opened up collections very slowly. If you had this problem, please try out this new version.
...
- Moved [RxQuery](../rx-query.md) checks into dev-mode.
