...
image: /headers/14.0.0.jpg
---

# 14.0.0

The release [14.0.0](https://rxdb.info/releases/14.0.0.html) is used for major refactorings and API changes.
The [replication](../replication.md) or the storage layer have only been touched marginally.
...
The PouchDB [RxStorage](../rx-storage.md) was deprecated in 13 and has now finally been removed, see [here](../rx-storage-pouchdb.md).
...
This feature was great when you use a [RxDocument](../rx-document.md) with some change-detection like in angular or vue templates. You can use document properties directly in the template and all updates will be reflected in the view, without having to use observables or subscriptions.
...
In the past, to start a replication, the replication plugin was added to RxDB and a method on the [RxCollection](../rx-collection.md) was called like `myRxCollection.syncGraphQL()`.
...
To make it possible to have alternative [encryption](../encryption.md) plugins, the `encryption` plugin was renamed to `encryption-crypto-js`.
...
Instead we now use just use the [RxDatabase](../rx-database.md).token together with the revision height.
...
- ADD [dev-mode](../dev-mode.md) shows a `console.warn()` to ensure people do not use it in production.
...
- FIX SQLite must store [attachments](../rx-attachment.md) data as Blob instead of base64 string to reduce the database size.
- FIX CouchDB replication [conflict handling](../transactions-conflicts-revisions.md)
