...
But the time has shown, that the old replication protocol is a big downside of RxDB:
  ...
  - The replication protocol did not support [attachments](../rx-attachment.md) and was not designed to ever support them.
...
So in version `13.0.0` I replaced the whole replication plugins with a new replication protocol. The main goals have been:
  ...
  - Implement the [conflict resolution](../transactions-conflicts-revisions.md) into RxDB so that the **client resolves its own conflicts** and does not rely on the backend.
  - Decrease the complexity for a compatible backend implementation. The new protocol relies on a *dumb* backend. This will open compatibility with many other use cases like implementing [Offline-First in Supabase](https://github.com/supabase/supabase/discussions/357) or using CouchDB but having a faster replication compared to the native [CouchDB replication](../replication-couchdb.md).
...
- REFACTORED the [encryption](../encryption.md) plugin, it is no longer a plugin but now a wrapper around any other RxStorage.
...
- REMOVED [RxDatabase](../rx-database.md).broadcastChannel The broadcast channel has been moved out of the RxDatabase and is part of the RxStorage. So it is no longer exposed via `RxDatabase.broadcastChannel`.
