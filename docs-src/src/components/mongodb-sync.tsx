...
export function RxdbMongoDiagramPlain({
  ...
  // When server is hidden, arrows should span from column 2 directly to the DB (column 5)
  const arrowToDbBase = showServer
    ? {}
    : {
      // span across columns 2,3,4 (up to the DB at col 5)
      gridColumn: '2 / 5' as React.CSSProperties['gridColumn'],
      // make the arrow fill its grid area and remove extra margins
      width: '90%',
      marginLeft: '5%',
      marginRight: 0,
    };
  ...
  `}</style>
      ...
      <div
        ...
        aria-label={
          showServer
            ? 'Clients to RxServer to MongoDB diagram'
            : 'Clients to MongoDB diagram'
        }
      >
        <div className="grid">
          {/* Client A */}
          <div className="box" style={{ gridColumn: 1, gridRow: 1 }}>
            <img src={logoUrl} alt="" className="logo" aria-hidden />
            {clientLabels[0] || 'Client A'}
          </div>
          <div
            className="arrow"
            style={{
              gridColumn: showServer ? 2 : (arrowToDbBase.gridColumn as any),
              gridRow: 1,
              ...(showServer ? {} : { width: '90%', marginLeft: '5%', marginRight: 0 }),
            }}
            aria-hidden
          />

          {/* Server (optional, full height) */}
          {showServer && (
          ...
          <div className="box db" style={{ gridColumn: 5 }}>
            <img src={dbIcon} alt="" className="logo" aria-hidden />
            {dbLabel}
          </div>
