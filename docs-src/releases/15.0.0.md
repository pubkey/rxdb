...
- Removed the browserify build output. We no longer ship `dist/rxdb.browserify.js` and `dist/rxdb.browserify.min.js`. If you need these, build them by yourself.
...
- REMOVED `RxChangeEvent.eventId`. If you really need a unique ID, you can craft your own one like:
```ts
[
    ev.documentId,
...
- The following things move out of beta:
