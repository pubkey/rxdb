...
var RxStorageInstanceMongoDB = /*#__PURE__*/function () {
  ...
  function RxStorageInstanceMongoDB(storage, databaseName, collectionName, schema, internals, options, settings) {
    ...
    this.runningWrites = new _rxjs.BehaviorSubject(0);
    ...
    this.mongoClient = new _mongodb.MongoClient(storage.databaseSettings.connection);
  ...
  _proto.bulkWrite = async function bulkWrite(documentWrites, context) {
    ...
    await (0, _rxjs.firstValueFrom)(this.runningWrites.pipe((0, _rxjs.filter)(c => c === 0)));
    ...
    var categorized = (0, _rxStorageHelper.categorizeBulkWriteRows)(this, primaryPath, documentStates, documentWrites, context);
    ...
    Promise.all(categorized.bulkInsertDocs.map(async writeRow => {
      ...
      if (writeResult.value) {
        ...
        var conflictError = {
          ...
          documentId: docId,
          writeRow,
          documentInDb: (0, _mongodbHelper.swapMongoToRxDoc)(writeResult.value),
          isError: true
        };
    ...
    Promise.all(categorized.bulkUpdateDocs.map(async writeRow => {
      ...
      var writeResult = await mongoCollection.findOneAndReplace({
        [this.inMongoPrimaryPath]: docId,
        _rev: (0, _utils.ensureNotFalsy)(writeRow.previous)._rev
      ...
      if (!writeResult.value) {
        ...
        var currentDoc = (0, _utils.getFromObjectOrThrow)(currentDocState, docId);
        // had insert conflict
        var conflictError = {
          status: 409,
          documentId: docId,
        ...
        var event = (0, _utils.getFromMapOrThrow)(categorized.changeByDocId, docId);
        eventBulk.events.push(event);
        ret.success[docId] = writeRow.document;
      }
    }))]);
  ...
  _proto.findDocumentsById = async function findDocumentsById(docIds, withDeleted, session) {
    ...
    queryResult.forEach(row => {
      result[row[primaryPath]] = (0, _mongodbHelper.swapMongoToRxDoc)(row);
    });
    this.runningOperations.next(this.runningOperations.getValue() - 1);
    return result;
  };
  ...
  _proto.getChangedDocumentsSince = async function getChangedDocumentsSince(limit, checkpoint) {
    ...
    var lastDoc = (0, _utils.lastOfArray)(documents);
    this.runningOperations.next(this.runningOperations.getValue() - 1);
    return {
  ...
  _proto.close = async function close() {
    ...
    await (0, _utils.requestIdlePromise)(200);
    if (this.closed) {
      return Promise.reject(new Error('already closed'));
    }
    this.closed = true;
