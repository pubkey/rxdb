import { clone, ensureNotFalsy, lastOfArray } from "../utils/index.js";
import { mongodbDocToRxDB } from "./mongodb-helper.js";
export async function getCurrentResumeToken(mongoCollection) {
...
export async function getDocsSinceChangestreamCheckpoint(primaryPath, mongoCollection,
/**
    ...
    var _loop = async function () {
      ...
      if (change) {
        ...
        if (change.operationType === 'delete') {
          var beforeDocMongo = ensureNotFalsy(change.fullDocumentBeforeChange, 'change must have pre-deletion state');
          var beforeDoc = mongodbDocToRxDB(primaryPath, beforeDocMongo);
          ...
          resultByDocId.set(docId, mongoCollection.findOne({
            ...
            if (doc) {
              ...
              var docFromChange = ensureNotFalsy(change.fullDocument, 'change must have change.fullDocument');
              var ret = mongodbDocToRxDB(primaryPath, docFromChange);
              ret._deleted = true;
              return ret;
            }
          }));
...
export async function getDocsSinceDocumentCheckpoint(primaryPath, mongoCollection, limit, checkpointId) {
  ...
  return docs.map(d => mongodbDocToRxDB(primaryPath, d));
}
export async function iterateCheckpoint(primaryPath, mongoCollection, limit, checkpoint) {
  if (!checkpoint) {
