 ...
 */
export async function migrateStorage(database,
/**
...
export async function migrateCollection(collection, oldDatabaseName, oldStorage, batchSize, afterMigrateBatch,
// to log each step, pass console.log.bind(console) here.
logFunction) {
  ...
  var _loop = async function () {
      ...
      if (schema.attachments) {
        await Promise.all(docs.map(async doc => {
          ...
          await Promise.all(Object.entries(doc._attachments).map(async ([attachmentId, attachmentMeta]) => {
            ...
            var attachmentDataString = await blobToBase64String(attachmentData);
      ...
      var writeToOldRows = docs.map((_doc, idx) => {
        ...
        newDoc._rev = createRevision(oldDatabaseInstanceToken, previous);
        return {
          previous,
          document: newDoc
        };
