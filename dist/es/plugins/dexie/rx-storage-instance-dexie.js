...
export var RxStorageInstanceDexie = /*#__PURE__*/function () {
      ...
      return Promise.resolve(_this.internals).then(function (state) {
        ...
        return Promise.resolve(state.dexieDb.transaction('rw', state.dexieTable, state.dexieDeletedTable, function () {
          try {
            ...
            return Promise.resolve(getDocsInDb(_this.internals, documentKeys)).then(function (docsInDbWithInternals) {
          ...
          if (ensureNotFalsy(categorized).eventBulk.events.length > 0) {
            var lastState = getNewestOfDocumentStates(_this.primaryPath, Object.values(ret.success));
            ...
            _this.changes$.next(ensureNotFalsy(categorized).eventBulk);
          }
          return ret;
        });
      });
      ...
      return Promise.resolve(_this2.internals).then(function (state) {
        ...
        return Promise.resolve(state.dexieDb.transaction('r', state.dexieTable, state.dexieDeletedTable, function () {
          try {
            ...
            var _temp = function () {
              if (deleted) {
                return Promise.resolve(getDocsInDb(_this2.internals, ids)).then(function (_getDocsInDb) {
  ...
  _proto.count = function count(preparedQuery) {
    try {
      var _this3 = this;
      return Promise.resolve(dexieCount(_this3, preparedQuery)).then(function (result) {
        return {
          count: result,
          mode: 'fast'
        };
    ...
    try {
      ...
      return Promise.resolve(_this4.internals).then(function (state) {
        return Promise.resolve(Promise.all([state.dexieTable, state.dexieDeletedTable].map(function (table) {
          ...
          changedDocs = sortDocumentsByLastWriteTime(_this4.primaryPath, changedDocs);
