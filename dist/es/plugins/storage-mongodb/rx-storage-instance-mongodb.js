...
import { ensureNotFalsy, getFromMapOrThrow, getFromObjectOrThrow, isMaybeReadonlyArray, lastOfArray, now, requestIdlePromise, RX_META_LWT_MINIMUM } from '../../plugins/utils';
import { MongoClient } from 'mongodb';
import { categorizeBulkWriteRows } from '../../rx-storage-helper';
import { MONGO_ID_SUBSTITUTE_FIELDNAME, getMongoDBIndexName, swapMongoToRxDoc, swapRxDocToMongo } from './mongodb-helper';
export var RxStorageInstanceMongoDB = /*#__PURE__*/function () {
  ...
  function RxStorageInstanceMongoDB(storage, databaseName, collectionName, schema, internals, options, settings) {
    ...
    this.runningWrites = new BehaviorSubject(0);
  ...
  _proto.bulkWrite = async function bulkWrite(documentWrites, context) {
    ...
    var categorized = categorizeBulkWriteRows(this, primaryPath, documentStates, documentWrites, context);
    ...
    Promise.all(categorized.bulkUpdateDocs.map(async writeRow => {
      ...
      if (!writeResult.value) {
        ...
        var currentDoc = getFromObjectOrThrow(currentDocState, docId);
        // had insert conflict
        var conflictError = {
          status: 409,
          documentId: docId,
  ...
  _proto.findDocumentsById = async function findDocumentsById(docIds, withDeleted, session) {
    ...
    queryResult.forEach(row => {
      result[row[primaryPath]] = swapMongoToRxDoc(row);
    });
    this.runningOperations.next(this.runningOperations.getValue() - 1);
    return result;
  };
