...
describe('rx-query.test.ts', () => {
    ...
    describeParallel('issues', () => {
         ...
         * deadline (2 s). With the old retry loop the test would time out.
         */
        it('#7067 query.$ emits first result within deadline under continuous concurrent writes', async () => {
            const memStorage = getRxStorageMemory();

            /**
             * Wrap query() so each call takes ~10 ms, simulating a real
             * async storage (Dexie/IndexedDB) where reads are never instant.
             * This makes concurrent writes reliably advance the counter before
             * the query returns, triggering the counter-mismatch code path.
             */
            const slowStorage = Object.assign({}, memStorage, {
                name: 'slow-' + memStorage.name,
                async createStorageInstance(params: any) {
                    const instance = await memStorage.createStorageInstance(params);
                    if (params.collectionName === 'humans') {
                        const orig = instance.query.bind(instance);
                        instance.query = (preparedQuery: any) =>
                            new Promise(res => setTimeout(() => res(orig(preparedQuery)), 10));
                    }
                    return instance;
                }
            });
            ...
            const db = await createRxDatabase({
                name: randomToken(10),
                storage: wrappedValidateAjvStorage({ storage: slowStorage }),
                eventReduce: false
            });
            const { humans: c } = await db.addCollections({
                humans: { schema: schemas.human }
            });

            // Pre-insert one document so the subscription has something to emit.
            await c.insert(schemaObjects.humanData('seed'));

            // Fire concurrent writes without awaiting each one so they truly
            // overlap with the subscription initialisation and advance the
            // ChangeEventBuffer counter while _execOverDatabase is in flight.
            // A fixed number of inserts keeps teardown bounded.
            const WRITE_COUNT = isFastMode() ? 10 : 30;
            const writePromises = Array.from({ length: WRITE_COUNT }, () =>
                c.insert(schemaObjects.humanData(randomToken(10))).catch(() => { /* ignore teardown errors */ })
            );

            // Give the first writes a tiny head-start before subscribing.
            await promiseWait(5);

            let firstEmissionDocCount = -1;
            const DEADLINE_MS = 2000;
            const firstEmissionReceived = new Promise<void>(res => {
                const sub = c.find().$.subscribe(docs => {
                    if (firstEmissionDocCount < 0 && docs.length > 0) {
                        firstEmissionDocCount = docs.length;
                        sub.unsubscribe();
                        res();
                    }
                });
            ...
            // Assert that the first emission arrives within the deadline.
            let timedOut = false;
            await Promise.race([
                firstEmissionReceived,
                promiseWait(DEADLINE_MS).then(() => {
                    timedOut = true;
                })
            ]);

            // Wait for all writes to finish before tearing down.
