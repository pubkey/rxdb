...
describe('bug-report.test.js', () => {
    describe('Pouchdb-server replication', () => {
        it('PouchDB-server: should fail with ensureNotFalsy() is falsy', async () => {
            /**
            ...
            await SpawnServer.spawn('mycollection', 10102);
            ...
            // Edit as needed
            const syncURL = '127.0.0.1:10102/db';
            const username = 'root';
            const password = 'root';
            const replicationState = replicateCouchDB({
                // replicationIdentifier: 'replicate-' + name,
                url: `http://${syncURL}` + '/mycollection' + '/',
    ...
    describe('CouchDB replication', () => {
        it('should fail with ensureNotFalsy() is falsy', async () => {
            /**
             * If your test should only run in nodejs or only run in the browser,
             * you should comment in the return operator and adapt the if statement.
             */
