...
describeParallel('query-planner.test.js', () => {
    ...
    describe('issues', () => {
        ...
        it('query with $gt/$lt on indexed field before a boolean field should not include boundary documents', async () => {
            const mySchema = {
                version: 0,
                primaryKey: 'id',
                type: 'object',
            ...
            const col = collections.mycollection;
            ...
            /**
             * Query with exclusive bounds on age.
             * Only docs with 20 < age < 50 should be returned (age=30 docs).
             * Previously, the upper bound index string for the boolean field
             * 'isActive' incorrectly mapped INDEX_MIN to '1' instead of '0',
             * which widened the index range and included docs with age=50
             * when selectorSatisfiedByIndex was true.
             */
            ...
            const resultIds = results.map((d: any) => d.id).sort();
            assert.deepStrictEqual(
                resultIds,
                ['b', 'c'],
                'Should only include documents with 20 < age < 50, got ids: ' + resultIds.join(', ')
            );

            // Also test count() which has a separate fast path
            const count = await col.count({
                selector: {
                    age: { $gt: 20, $lt: 50 }
                }
            }).exec();
            assert.strictEqual(count, 2, 'Count should be 2 for 20 < age < 50, got ' + count);
