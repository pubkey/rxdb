...
describeParallel('query-planner.test.js', () => {
    ...
    describe('always prefer the better index', () => {
        ...
        it('should return correct results when enum field with $eq is sort-irrelevant', async () => {
            const mySchema = {
                version: 0,
                primaryKey: 'id',
                type: 'object',
            ...
            const preparedQuery = await query.getPreparedQuery();
            assert.ok(preparedQuery.queryPlan.sortSatisfiedByIndex);

            const results = await query.exec();
            assert.strictEqual(results.length, 3);
            assert.deepStrictEqual(
                results.map(d => d.toJSON(true).id),
                ['3', '1', '4']
            );
            assert.deepStrictEqual(
                results.map(d => d.toJSON(true).age),
                [10, 30, 50]
            );
