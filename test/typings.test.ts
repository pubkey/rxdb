            ...
            it('typed collection should know its static orm methods', async () => {
                const code = codeBase + `
                    (async() => {
                        ...
                        const col: RxCollection<any, any, staticMethods> = myCollections.humans;
            ...
            it('UNTYPED collection should allow to access any static orm-method', async () => {
                const code = codeBase + `
                    (async() => {
                        ...
                        await myCollections.humans.countAllDocuments();
                    })();
                `;
                await transpileCode(code);
            });
        ...
        it('.atomicUpdate()', async () => {
            const code = codeBase + `
                (async() => {
                    ...
                    const doc = await myCollections.humans.findOne().exec();
                    if(!doc) return;
        ...
        describe('hooks', () => {
            it('should know the types', async () => {
                ...
                (async() => {
                    ...
                    let myNumber: number;
                    let myString: string;
                    myCollections.humans.postInsert((data, doc) => {
                            myNumber = doc.age;
                            myNumber = data.age;
                            myString = doc.foobar();
