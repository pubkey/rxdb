...
> TanStack DB delegates sync to its collection. Learn how to sync local data to and from any backend using RxDB's mature replication protocol under TanStack DB.
...
function TodoList() {
    ...
    return {openTodos.map(todo => {todo.text})};
}
